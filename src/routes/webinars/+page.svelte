...
<main class="font-poppins bg-[var(--webinar-secondary)] text-[var(--webinar-secondary-foreground)] min-h-screen webinar-page">
  ...
  <section class="relative min-h-screen flex items-center justify-center overflow-hidden py-20">
    ...
    <div class="container mx-auto px-4 relative z-10 text-center max-w-4xl">
      <h1 class="text-4xl md:text-6xl font-bold mb-4">
        SESIÓN ESTRATÉGICA</h1>
      <h1><span class="text-[var(--webinar-primary)]">DE EXPANSIÓN</span></h1>
    ...
    <div class="container mx-auto px-4">
      ...
      <div class="grid md:grid-cols-2 gap-8">
        {#each benefits as benefit, index}
          <div class="bg-[var(--gray-dark)] p-8 rounded-lg flex items-start gap-4" in:fly={{y: 20, duration: 500, delay: index * 200}}>
            <span class="text-[var(--blue)] text-xl">🔹</span>
            <div>
              <h3 class="text-xl font-bold mb-2 text-[var(--webinar-primary)]">{benefit.title}</h3>
              <p class="text-[var(--gray-light)]">{benefit.description}</p>
            </div>
          </div>
        {/each}
      </div>
  ...
  <section class="py-20">
    <div class="container mx-auto px-4">
      ...
      <div class="grid md:grid-cols-2 gap-12 max-w-5xl mx-auto">
        <div class="bg-[var(--gray-dark)] p-8 rounded-lg">
          <h3 class="text-2xl font-bold mb-6 text-[var(--webinar-primary)] flex items-center">✔️ SÍ, ES PARA TI SI...</h3>
          <ul class="space-y-4">
            {#each forYouPoints as point}
              <li class="flex items-start gap-3"><span class="text-[var(--webinar-primary)]">✔️</span><span>{point}</span></li>
            {/each}
          </ul>
        </div>
        <div class="bg-[var(--gray-dark)] p-8 rounded-lg">
          <h3 class="text-2xl font-bold mb-6 text-[var(--red)] flex items-center">❌ NO ES PARA TI SI...</h3>
          <ul class="space-y-4">
            {#each notForYouPoints as point}
              <li class="flex items-start gap-3"><span class="text-[var(--red)]">❌</span><span>{point}</span></li>
            {/each}
          </ul>
        </div>
      </div>
  ...
  <section class="py-20">
    <div class="container mx-auto px-4">
      ...
      <div class="max-w-3xl mx-auto bg-[var(--gray-dark)] p-8 rounded-lg text-center">
        <div class="flex justify-center mb-4">{#each Array(5) as _}<span class="text-[var(--webinar-primary)] text-xl">⭐</span>{/each}</div>
        <p class="text-lg mb-6 italic">"{testimonials[0].text}"</p>
        <p class="font-semibold text-[var(--webinar-primary)]">{testimonials[0].name}</p>
        <p class="text-sm text-[var(--gray-role)]">{testimonials[0].role}</p>
      </div>
    </div>
  </section>
  ...
  <section class="py-20 bg-[var(--cta-bg)]">
    <div class="container mx-auto px-4 text-center">
      ...
      <p class="text-2xl font-bold text-[var(--webinar-primary)] mb-8">Comienza hoy.</p>
    </div>
  </section>
